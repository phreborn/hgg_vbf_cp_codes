...
void getMag_w1(int sampleID, int iSysInit = 1, int iSysFin = 1, TString sysSet = "photonsys", bool doSys = true){
  ...
  for(auto id : v_mcID){
    ...
    TString dirName = basepath+"csv_w1/"+TString(Form("Collect_%s_%i_%i", sysSet.Data(), iSysInit, iSysFin));
    ...
    for(auto cat : catCuts){
      for(auto d = d_tmp.begin(); d != d_tmp.end(); d++){
        ofstream ofsyst(Form(basepath+"csv_w1/Collect_%s_%i_%i/mag_yield_%i_"+d->first+"_"+cat.first+".csv", sysSet.Data(), iSysInit, iSysFin, mcID), ios::out);
        if(!ofsyst){
          ofsyst.close();
    ...
    for(auto cat : catCuts){
      for(auto d = d_tmp.begin(); d != d_tmp.end(); d++){
        ...
        ofstream yofsyst(Form(basepath+"csv_w1/Collect_%s_%i_%i/yield_%i_"+d->first+"_"+cat.first+".csv", sysSet.Data(), iSysInit, iSysFin, mcID), ios::out);
        if(!yofsyst){
          yofsyst.close();
    ...
    // fill nominal yields
    ofstream ofyield_clear(Form(basepath+"csv_w1/Collect_%s_%i_%i/N_yield.csv", sysSet.Data(), iSysInit, iSysFin), ios::app);
    ofyield_clear.close();
    ofstream ofyield(Form(basepath+"csv_w1/Collect_%s_%i_%i/N_yield.csv", sysSet.Data(), iSysInit, iSysFin), ios::app);
    if(!ofyield){
      ofyield.close();
      cout<<"error can't open file for record"<<endl;
    }
...
int main(int argc, char* argv[]){
  ...
  getMag_w1(sampleID,init,fin,sysSet);
  return 0;
}
