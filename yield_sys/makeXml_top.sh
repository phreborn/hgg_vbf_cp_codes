...
#for d in m00;do
  ...
  if [ $injectTest -eq 1 ];then
    ...
    echo "  <POI>mu_VBF_RW,mu_VBF_SM,mu_ggH,mu_ggH_SM,mu_spur,mu_spur_SM,mu</POI>" >> $out_xml
  else
...
#    echo "  <Asimov Name=\"toyFit\"   Setup=\"mu=1,mu_VBF_RW=1_0_5,mu_ggH=1,mu_VBF_SM=0,mu_ggH_SM=0\"     Action=\"fixsyst:fit:float:savesnapshot\" SnapshotNuis=\"nominalNuis_toyFit\" SnapshotGlob=\"nominalGlob_toyFit\"/>" >> $out_xml
    ...
    echo "  <Asimov Name=\"asimovData_SM_floatMu\"   Setup=\"mu=1,mu_ggH=1,mu_VBF_RW=1_0_5,mu_ggH_SM=0,mu_VBF_SM=0\"     Action=\"float:fit:genasimov:savesnapshot\" SnapshotNuis=\"Nuis_SM_floatMu\" SnapshotGlob=\"Glob_SM_floatMu\"/>" >> $out_xml
    echo "  <Asimov Name=\"asimovData_SM_Mu1\"   Setup=\"mu=1,mu_ggH=1,mu_VBF_RW=1,mu_ggH_SM=0,mu_VBF_SM=0\"     Action=\"genasimov:float:savesnapshot:raw\" SnapshotNuis=\"Nuis_SM_Mu1\" SnapshotGlob=\"Glob_SM_Mu1\"/>" >> $out_xml
  elif [ ${unblind} -eq 1 -a ${unblindAsi} -eq 2 ];then
    echo "  <Asimov Name=\"asimovData_SM_floatMu\"   Setup=\"mu=1,mu_spur=0,mu_ggH=0,mu_VBF_RW=0,mu_spur_SM=1,mu_ggH_SM=1,mu_VBF_SM=1_0_5\"     Action=\"float:fit:genasimov:savesnapshot\" SnapshotNuis=\"Nuis_SM_floatMu\" SnapshotGlob=\"Glob_SM_floatMu\"/>" >> $out_xml
    echo "  <Asimov Name=\"asimovData_SM_Mu1\"   Setup=\"mu=1,mu_spur=0,mu_ggH=0,mu_VBF_RW=0,mu_spur_SM=1,mu_ggH_SM=1,mu_VBF_SM=1\"     Action=\"genasimov:matchglob:savesnapshot:reset:raw\" SnapshotNuis=\"Nuis_SM_Mu1\" SnapshotGlob=\"Glob_SM_Mu1\"/>" >> $out_xml
  fi
...
#    echo "  <Asimov Name=\"asimovData_SB_SM\"  Setup=\"mu=1,mu_VBF_RW=0,mu_ggH=0,mu_bkg=1,mu_bkg_funcBias_Asi=0\"     Action=\"genasimov:reset\"/>" >> $out_xml
    ...
    echo "  <Asimov Name=\"asimovData_SB_SM\"  Setup=\"mu=1,mu_ggH=0,mu_VBF_RW=0,mu_ggH_SM=1,mu_VBF_SM=1\"     Action=\"genasimov:reset\"/>" >> $out_xml
  fi
